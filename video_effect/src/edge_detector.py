...
def get_adjacent_pixels(image, row_number, col_number, num_of_rows, num_of_cols):
	mat = np.zeros((3,3))
	for i in range(3):
...
def get_edge_map(image):
	...
	for row_number in range(num_of_rows):
		for col_number in range(num_of_cols):
			adjacent_pixels = get_adjacent_pixels(image, row_number, col_number, num_of_rows, num_of_cols)
